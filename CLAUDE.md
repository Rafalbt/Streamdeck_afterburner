...
**If the plugin doesn't appear in Stream Deck**, run `npm run validate` first — Stream Deck silently refuses to load a plugin whose manifest fails validation. Then `npm run restart`, and check `com.hwinfi.afterburner.sdPlugin/logs/` for the plugin process output.

**Dependencies install in two places**: `npm install` at the repo root (SDK, esbuild, TS), *and* `npm install` inside `com.hwinfi.afterburner.sdPlugin/` (installs `koffi` so it resolves at runtime next to the bundled `bin/plugin.js`, and ships with a packaged plugin).
...
```bash
npx esbuild com.hwinfi.afterburner.sdPlugin/src/test-mahm.ts --bundle --platform=node \
  --target=node20 --format=cjs --external:koffi \
  --outfile=com.hwinfi.afterburner.sdPlugin/bin/test-mahm.cjs
node com.hwinfi.afterburner.sdPlugin/bin/test-mahm.cjs   # must run from a dir where koffi resolves
```
...
### Files (`com.hwinfi.afterburner.sdPlugin/src/`)
...
- **Manifest requires a top-level `UUID`** (`com.hwinfi.afterburner`) matching the `.sdPlugin` folder, and `URL` must be a valid http(s) URL or omitted entirely — an empty string fails validation.
